...
def hash_state_dict(state_dict, device='cpu'):
        ...
        return m.hexdigest()
class Exp_Imputation_0(Exp_Basic):
    def __init__(self, args):
        super(Exp_Imputation_0, self).__init__(args)
        self.loss=args.loss
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            epoch_time = time.time()
            for i, (batch_x, batch_x_e, batch_y, pos_s,pos_e,datee,mask) in enumerate(train_loader):
                #print(batch_x.shape,batch_x_e.shape,batch_y.shape,pos_s.shape,pos_e.shape,datee.shape,mask.shape)
                if i%100 == 0:
                    print(i)
                if i == 0:
                    batch_hash = get_batch_hash(batch_x)
