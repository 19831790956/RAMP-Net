...
class Frequency_Aware_Delay_Attn(nn.Module):
    def __init__(self, args,M):
        ...
        self.dropout = nn.Dropout(0.1)
        self.attn_mask=None
        self.M_self_real=nn.Parameter(torch.randn(1, self.seq_len)).to(self.device)
        self.M_self_image=nn.Parameter(torch.randn(1, self.seq_len)).to(self.device)
        M_real=nn.Parameter(torch.randn(M, self.seq_len)).to(self.device)
        M_imag=nn.Parameter(torch.randn(M, self.seq_len)).to(self.device)
    ...
    def forward(self,xp):
        ...
        xp_self=xp
        temp=self.temp.repeat(B,C,N,1,1)#B,C,N,M,L
        ...
        #attn:Q(B,C,N,D),K,V(B,C,N,M,D)
        xp_emb_real=xp_emb.real
        xp_emb_imag=xp_emb.imag
        xp_temp_emb_real=xp_temp_emb.real
        xp_temp_emb_imag=xp_temp_emb.imag
        ...
        xp_self_real=self.M_self_real.repeat(B,C,N,1)*xp_self.real
        xp_self_image=self.M_self_image.repeat(B,C,N,1)*xp_self.imag
        xp_self=torch.complex(xp_self_real,xp_self_image)
        output_2=torch.fft.irfft(xp_self, dim=-1)
        output_=torch.cat([output,output_2],dim=1)
        output=self.predict_layer(output_)
